...
from . import BlenderImage, ShaderNodes
from .idtech3lib import ID3Shader
from .idtech3lib.Parsing import *
from .idtech3lib.ImportSettings import NormalMapOption, Preset
...
class quake_shader:
        ...
        if alphaGen == ALPHA_UNDEFINED:
            ...
            if shader.is_grid_lit:
                shader.links.new(shader.get_rgbGen_node(
                    LIGHTING_LIGHTGRID).outputs["LightGridVector"], # type: ignore
                    spec_node.inputs["LightVector"]) # type: ignore
            else:
                shader.links.new(shader.get_node_by_name(
                    "BaseReflectionVector").outputs["Vector"], # type: ignore
                    spec_node.inputs["LightVector"]) # type: ignore
            return spec_node
        else:
    ...
    def get_tcMod_node(shader, tcMods, tcMod_arguments):
        ...
        for tcMod, arguments in zip(tcMods, tcMod_arguments):
            if tcMod == "scale":
                ...
                shader.links.new(
                    time_node.outputs["Time"], # type: ignore
    ...
    def build_stage_nodes(shader, VFS, stage: vanilla_shader_stage, color_out, alpha_out):
        ...
        if stage.valid:
            ...
            if stage.detail or stage.blend != BLEND_NONE:
            ...
            if img is not None:
                ...
                if stage.lighting == LIGHTING_CONST:
                    lighting.outputs[0].default_value = ( # type: ignore
    ...
    def finish_rendering_shader(shader, VFS, import_settings):
        ...
        if (shader.is_system_shader or 
            ...
            for stage in shader.stages:
                if stage.alpha == ALPHA_SPEC and stage.blend == "gl_src_alpha gl_one":
                    img = bpy.data.images.get(stage.diffuse)
                    if img is None:
                        img = BlenderImage.load_file(stage.diffuse, VFS)
                    if img is not None:
                        node_color = shader.nodes.new(type='ShaderNodeTexImage')
                        node_color.image = img
                        node_color.image.colorspace_settings.name = "Non-Color"
                        if stage.clamp:
                            node_color.extension = 'EXTEND'
                        node_color.location = shader.current_x_location + 200, shader.current_y_location
                        out_Shine = node_color.outputs["Color"]
                    break
            ...
            if out_Color is not None:
                ...
                if out_Shine is not None:
                    node_invert = shader.nodes.new(type="ShaderNodeInvert")
                    node_invert.location = (node_BSDF.location[0] - 200, node_BSDF.location[1] - 300)
                    shader.links.new(out_Shine, node_invert.inputs["Color"])
                    shader.links.new(node_invert.outputs["Color"], node_BSDF.inputs["Roughness"])
                else:
                ...
                if out_Glow is not None or node_light is not None:
                    ...
                    shader.links.new(
                        new_node.outputs[0], node_BSDF.inputs[EMISSION_KEY]) # type: ignore
                    if out_Glow is not None:
                        shader.links.new(out_Glow, new_node.inputs[2]) # type: ignore
                    if node_light is not None:
                        shader.links.new(
                            node_light.outputs[0], new_node.inputs[0]) # type: ignore
                    if "q3map_surfacelight" in shader.attributes:
                        try:
                            new_node.inputs[1].default_value = ( # type: ignore
                ...
                if out_Glow is not None or node_light is not None:
                    new_node = shader.get_node_by_name("EmissionScaleNode")
                    shader.links.new(
                        new_node.outputs[0], node_Emiss.inputs["Color"]) # type: ignore
                    if out_Glow is not None:
                        shader.links.new(out_Glow, new_node.inputs[2]) # type: ignore
                    if node_light is not None:
                        shader.links.new(
        ...
        if node_BSDF is None and node_BSDF.inputs.get("Normal"):
            ...
            normal_node.location = 700, -500 # type: ignore
            shader.links.new(
                normal_node.outputs[0], node_BSDF.inputs["Normal"]) # type: ignore
                ...
                if shader.is_vertex_lit and stage.lightmap:
                    if color_out is None:
                        color_out = shader.get_rgbGen_node(
                            LIGHTING_VERTEX).outputs[0] # type: ignore
                    elif shader.last_blend is not None:
                        shader.links.new(
                            shader.get_rgbGen_node(LIGHTING_VERTEX).outputs[0], # type: ignore
                            shader.last_blend.inputs['rgbGen'])
                elif shader.is_grid_lit and stage.lightmap:
                    if color_out is None:
                        color_out = shader.get_rgbGen_node(
                            LIGHTING_LIGHTGRID).outputs[0] # type: ignore
                    elif shader.last_blend is not None:
                        shader.links.new(
            ...
            if img is not None:
                ...
                if shader.is_vertex_lit:
                    ...
                    if lightmap is None:
                        ...
                        node_lm.location = 1200, -800 # type: ignore
                    else:
                        node_lm = shader.nodes.new(type='ShaderNodeTexImage')
                    ...
                    if tc_gen is not None and lightmap is not None:
                        shader.links.new(
                            tc_gen.outputs["UV"], node_lm.inputs["Vector"]) # type: ignore
                    ...
                    shader.links.new(
                        ...
                        node_blend.inputs["Color1"])
                    shader.links.new(
                        node_lm.outputs["Color"], node_blend.inputs["Color2"]) # type: ignore
                    color_out = node_blend.outputs["Color"]
    ...
    def finish_shader(shader, VFS, import_settings):
        if (import_settings.preset != Preset.RENDERING.value):
